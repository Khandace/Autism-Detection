...
This process involves reducing the number of variables within the dataset, especially the ones that do not offer any significance to the prediction or do not wish to carry on with them, this can be termed variable reduction. Finally selected 15 variables
...
During this step and with the reaming 15 variables were used for the modelling by partitioning the data into two sets, that’s the training set and the testing set. 80% of the data was used for training and testing 20% reaming the first 487 rows in the data will be trained while the rest will be tested. 
...
# Run final model with all instances
...
Out of all 4 models, KNN predicted accuracy of 98% and a kappa of 96% with an error rate of 2%, this means that KNN has an excellent agreement in terms of truth values compared to the rest of the models. It can be said that KNN has proven to be the best model for detecting autism in adults in this paper.
